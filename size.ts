...
for (const [file, budgetKib] of Object.entries(BUDGETS_KIB)) {
  ...
  console.log(`${ok ? "ok  " : "OVER"} — ${file}: ${kib.toFixed(1)} KiB gzip (budget ${budgetKib} KiB)`);

  if (!ok) failed = true;
}

if (failed) {
  console.error("size: bundle(s) exceed the post-gzip budget (see MODERNISATION_REPORT.md §3, P3.9)");
  process.exit(1);
}
